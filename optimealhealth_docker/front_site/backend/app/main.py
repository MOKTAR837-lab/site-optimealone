...
from app.routers import menu, clients, food_vision, questionnaire, meal_plans, recipes
from app.services.geo_service import GeoService
from app.knowledge.cache import get_embedding_cache
from app.knowledge.search import hybrid_search

app = FastAPI(title="Optimeal API", version="0.2.0", docs_url="/docs", redoc_url="/redoc", openapi_url="/openapi.json")
...
async def geo_check(request: Request):
    ...
    if ip:
        ...
        ip = "UNKNOWN"
    country = await GeoService.get_country(ip)
    from app.config.geo_config import GeoConfig
    return {
        "your_ip": ip,
        "your_country": country,
        "optimealone": {"accessible": True},
        "optimealhealth": {"accessible": GeoConfig.check_access_optimealhealth(country)}
    }




# --- Fallback GeoConfig (safe) ---
try:
    _ = GeoConfig  # existe peut-être déjà
except NameError:
    class GeoConfig:  # minimal
        EU = {"FR","BE","CH","DE","IT","ES","PT","NL","LU","IE","GB","AT","SE","NO","DK","FI","PL","CZ","SK","HU","RO","BG","HR","SI","GR","EE","LV","LT","MT","CY"}
        @staticmethod
        def check_access_optimealone(country: str) -> bool:
            return True  # mondial
        @staticmethod
        def check_access_optimealhealth(country: str) -> bool:
            try:
                return (country or "").upper() in GeoConfig.EU
            except Exception:
                return False
